...
def create_or_update_user(user_id: int, user: schemas.UserCreate, db: Session = Depends(get_db)):
...
def create_device_usage(usage: schemas.DeviceUsageCreate, db: Session = Depends(get_db)):
    return crud.create_device_usage(db, usage)

@app.get("/device_usages/", response_model=list[schemas.DeviceUsage])
def read_device_usages(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
