...
def get_db_schema_prompt():
    ...
    try:
        ...
        schema_prompt = (
            "你是一名专业的智能家居数据分析师助手。"
            "请根据用户的自然语言需求和下方提供的数据库表结构，严格遵循规则，生成SQL或调用工具。\n\n"
            "【数据库表结构】:\n"
            f"{full_schema_str}\n\n"
            "【重要指导原则】:\n"
            ...
            "【可用工具】:\n"
            "1. `generate_visualization`: "
            "当用户的意图涉及到**分析、可视化、图表、趋势、分布、对比、占比、排行**等时，"
            "你**必须**优先调用此工具。"
            "此工具需要一个`title`（图表标题）和一条用于生成数据的`sql`查询。\n\n"
            "   **调用范例**:\n"
            "   - **用户输入**: `我想看看设备使用频率的图表`\n"
            "   - **你的输出**:\n"
            "   ```json\n"
            ...
            "     \"tool_input\": {\n"
            "       \"title\": \"设备使用频率分析\",\n"
            "       \"sql\": \"SELECT d.name, COUNT(u.id) AS usage_count "
            "FROM device_usages u JOIN devices d ON u.device_id = d.id "
            "GROUP BY d.name ORDER BY usage_count DESC;\"\n"
            "     }\n"
            "   }\n"
            "   ```\n\n"
            "【生成要求】:\n"
            "1. 如果决定调用工具，你的**全部回答**必须严格遵循上述JSON格式，"
            "**只能**输出JSON代码块，禁止包含任何额外的解释或文字。\n"
            "2. **仅当**用户进行简单的、非分析性的数据查询时（如'张三的id是多少？'），"
            "才直接生成SQL语句，并以分号结尾。\n"
...
async def nlp_query(request: Request, db: Session = Depends(get_db)):
    ...
    if not api_key:
        error_message = (
            f"未能找到模型 '{model_provider}' 的API Key。"
            f"请确保您已在 .env 文件中配置了 {model_provider.upper()}_API_KEY。"
        )
    ...
    sql_match = re.search(
        r'((select|insert|update|delete)[\s\S]+?;)', answer, re.IGNORECASE)
    if sql_match:
        ...
        try:
            ...
            if sql_type == 'select':
                ...
                last_user_question = user_messages[-1].get(
                    "content", "").lower() if user_messages else ""

                visualization_keywords = [
                    "图表", "可视化", "分析", "趋势", "分布", "对比", "占比", "排行",
                    "chart", "visualize", "analysis", "trend", "distribution",
                    "comparison", "ranking"
                ]

                # If the user's query contained visualization keywords and we have
                # data,
                # we override the LLM's formatting failure and force a visualization.
                if any(keyword in last_user_question for keyword in
                       visualization_keywords) and rows:
                    return {
                        "action": "visualize",
                        "data": rows,
